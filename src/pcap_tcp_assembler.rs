...
use crate::{consume_buffer::consume_buffer, tcp_connection::TcpConnection, tcp_receive_storage::TcpReceiveStorage, trace, trace_macro::{check_trace_connection}};
...
impl<'a> PcapTcpAssembler<'a> {
    ...
    pub fn handle_segment(
        ...
        // https://datatracker.ietf.org/doc/html/rfc1323 is not yet implemented.
        ...
        if tcp_storage_option.is_some() {
            ...
            if tcp_header_slice.sequence_number() < tcp_storage.last_consumed_sequence_number {
                ...
                const SEQUENCE_NUMBER_OLD_DUPLICATE_THRESHOLD: u32 = 65536;
                ...
                if sequence_number_rotated {
                    ...
                    if sequence_number_old_duplicate {
                        ...
                        trace!(
                            "[{}]: {} Raw sequence number is smaller, than the last_consumed_sequence_number {}, it is outside of 'old duplicate' threshold {}, reset connection.",
                            self.trace_connections,
                            tcp_storage.connection_id,
                            tcp_header_slice.sequence_number(),
                            tcp_storage.last_consumed_sequence_number,
                ...
                if sequence_number_outside_of_receive_buffer {
                    trace!(
                        "[{}]: {} Raw sequence number is outside of receive buffer {}, reset connection.",
                        self.trace_connections,
                        tcp_storage.connection_id,
                        tcp_header_slice.sequence_number(),
                        tcp_storage.last_consumed_sequence_number
